class FamilyIntercomPanel extends HTMLElement {
  set hass(hass) {
    ...
    if (!this._familyConfigRequested) this._loadFamilyConfig();
    this._updateTargets();
  ...
  _render() {
    ...
    this.innerHTML = `
      <style>
        :host{display:block;width:100%;height:100%;min-height:100dvh;box-sizing:border-box;padding:0 clamp(14px,3vw,28px) clamp(14px,3vw,28px);overflow-x:hidden;overflow-y:auto;-webkit-overflow-scrolling:touch;overscroll-behavior:contain;--fi-blue:#4f8cff;--fi-cyan:#23d5d5;--fi-purple:#9b5cff;--fi-pink:#ff4f9a;--fi-orange:#ffb347;--fi-red:#ef4444;--fi-green:#4ade80;--fi-dark:#101827}
        *{box-sizing:border-box}
        .wrap{width:100%;max-width:1240px;min-width:0;margin:0 auto;display:grid;gap:18px;padding-top:clamp(6px,1vw,12px)}
        ...
        @media (max-width:820px){.layout{grid-template-columns:1fr}.hero-content{grid-template-columns:1fr}.device-pill{width:max-content}.card{border-radius:20px}:host{height:100dvh;min-height:100dvh;overflow-y:auto;padding:0 calc(12px + env(safe-area-inset-right,0px)) calc(128px + env(safe-area-inset-bottom,0px)) calc(12px + env(safe-area-inset-left,0px))}.wrap{padding-top:6px}.toolbar{grid-template-columns:1fr}}
        @media (max-width:520px){
          :host{padding:0 calc(8px + env(safe-area-inset-right,0px)) calc(150px + env(safe-area-inset-bottom,0px)) calc(8px + env(safe-area-inset-left,0px))}
          .wrap{gap:10px;padding-top:4px}
          ...
          .device-pill{padding:10px 12px}
          .hero-stat{padding:12px;grid-template-columns:auto minmax(0,1fr);font-size:.82rem}
          .hero-stat strong{font-size:.95rem;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
      ...
      <div class="wrap${this._displayMode ? " display-mode" : ""}${this._replyMode ? " reply-mode" : ""}">
        ...
        <div class="layout">
          ...
          <aside class="card">
            ...
            <div class="section-head">
              <h2>Reply inbox</h2>
              <button id="refreshInbox" class="secondary" title="Refresh reply inbox">Refresh</button>
            </div>
            <div class="history-list" id="inboxList"></div>
            <h2>Available now</h2>
  ...
  _renderStationChips(players = this._players()) {
    const container = this.querySelector("#stationChips");
    if (!container) return;
    container.replaceChildren();
    if (!this._stations?.length) {
      const empty = document.createElement("span");
      empty.className = "mini";
      empty.textContent = "Configure stations in integration options.";
      container.append(empty);
      return;
    }
    const available = new Set(players.map(player => player.entityId));
  ...
  _renderQuickMessages() {
    ...
    for (const phrase of this._replyPhrases || []) {
      if (!quick.some(([message]) => message.toLowerCase() === String(phrase).toLowerCase())) quick.push([phrase, "secondary"]);
    }
  ...
  _renderInbox() {
    const container = this.querySelector("#inboxList");
    if (!container) return;
    container.replaceChildren();
    if (!this._inboxReplies?.length) {
      const empty = document.createElement("div");
      empty.className = "mini";
    ...
    for (const item of this._inboxReplies.slice(0, 8)) {
      const row = document.createElement("div");
      row.className = "history-item";
      const time = item.time ? new Date(item.time).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" }) : "";
      row.innerHTML = `
        <div class="history-message">${this._escape(item.message || "Reply")}</div>
        <div class="history-meta">${this._escape(item.status || "status unknown")} from ${this._escape(item.from || "Intercom")}${time ? " - " + this._escape(time) : ""}</div>
      `;
      container.append(row);
    }
  }
